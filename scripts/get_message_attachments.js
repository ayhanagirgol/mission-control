...
if (!MESSAGE_ID) {
  console.error('Usage: get_message_attachments.js <messageId>');
  process.exit(2);
}
...
(async () => {
  ...
  const url = `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(MAILBOX_UPN)}/messages/${encodeURIComponent(MESSAGE_ID)}/attachments?$select=id,name,contentType,size`;
  const data = await graph(token, url);
  console.log(JSON.stringify({ attachments: data?.value || [] }, null, 2));
})();
