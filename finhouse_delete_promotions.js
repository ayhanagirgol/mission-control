...
function classify(m) {
  ...
  if (from.includes('netflix') || subject.includes('netflix')) return 'netflix';
  if (from.includes('habitify') || subject.includes('habitify')) return 'habitify';
  return null;
}
...
async function main() {
  ...
  const matches = [];
  while (url && matches.length < 100) {
    const page = await graph(token, url);
    for (const m of (page.value || [])) {
      const kind = classify(m);
      if (kind) matches.push({ ...m, kind });
    }
  ...
  for (const m of matches) {
    try {
      ...
      deleted.push({ kind: m.kind, subject: m.subject, from: m.from?.emailAddress?.address, receivedDateTime: m.receivedDateTime });
    } catch (err) {
      errors.push({ kind: m.kind, subject: m.subject, error: String(err?.message || err) });
    }
  }
