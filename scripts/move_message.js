...
const DEST_FOLDER_ID = process.argv[3];

if (!MESSAGE_ID || !DEST_FOLDER_ID) {
  console.error('Usage: move_message.js <messageId> <destinationFolderId>');
  process.exit(2);
}
...
(async () => {
  ...
  const url = `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(MAILBOX_UPN)}/messages/${encodeURIComponent(MESSAGE_ID)}/move`;
  const moved = await graph(token, url, {
    method: 'POST',
    body: JSON.stringify({ destinationId: DEST_FOLDER_ID }),
  });
  console.log(JSON.stringify({ movedId: moved?.id }, null, 2));
})();
