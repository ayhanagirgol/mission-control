...
async function ensureFolder(token) {
  ...
  const json = await graph(token, listUrl, { method: 'GET' });
...
async function runSearch(token, searchValue) {
  const results = [];
...
async function searchEvVendorMessages(token) {
  const map = new Map();
  for (const term of SEARCH_TERMS) {
    const matches = await runSearch(token, term);
...
(async () => {
  ...
  const vendorMessages = await searchEvVendorMessages(token);
  ...
  const table = summary.length
    ...
    : 'Zeplin anahtar kelimesiyle eşleşen e-posta bulunamadı.';
  ...
  console.log(`Bulunan EV ödeme e-postası: ${summary.length}`);
  console.log(table);
})();
