...
if (!MESSAGE_ID) {
  console.error('Usage: delete_message.js <messageId>');
  process.exit(2);
}
...
async function deleteMessage() {
  ...
  const moveUrl = `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(MAILBOX_UPN)}/messages/${encodeURIComponent(MESSAGE_ID)}/move`;
  ...
  if (!newId) {
    console.warn('Message moved to Deleted Items but no ID returned; cannot hard delete.');
    return;
  }
