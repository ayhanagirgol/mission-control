/**
 * GoDaddy için inbox kuralı oluştur — gelecekte gelen mailleri otomatik GoDaddy klasörüne taşı
 */
...
// GoDaddy klasörünü bul
const fRes = await graph(token, `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(MAILBOX)}/mailFolders?$top=100`);
const folder = (fRes.value || []).find(f => f.displayName.toLowerCase() === 'godaddy');
if (!folder) throw new Error('GoDaddy klasörü bulunamadı!');

// Inbox kuralı oluştur
const rule = await graph(token,
  `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(MAILBOX)}/mailFolders/inbox/messageRules`,
  {
    method: 'POST',
    body: JSON.stringify({
      displayName: 'GoDaddy → GoDaddy Klasörü',
      sequence: 2,
      isEnabled: true,
      conditions: { senderContains: ['godaddy.com'] },
      actions: { moveToFolder: folder.id }
    })
  }
);

console.log(`✅ Kural oluşturuldu: ${rule.displayName} (id: ${rule.id})`);
console.log(`📁 Hedef klasör: GoDaddy (${folder.id})`);
console.log(`GoDaddy'den gelen tüm mailler artık otomatik GoDaddy klasörüne taşınacak.`);
