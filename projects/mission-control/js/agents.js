    ...
    <div class="card-grid">
      ${agents.map(a => {
        ...
        <div class="card agent-card" onclick="showAgentDetail('${a.id}')">
          ...
          ${a.model ? `<div class="agent-stats" style="margin-top:0.5rem;font-size:0.75rem;color:var(--text-muted);display:flex;justify-content:space-between;">
            <span>Model: ${a.model}</span>
            <span>Tokens: ${a.totalTokens || '0'}</span>
            ${a.estimatedCostUsd ? `<span>Cost: $${a.estimatedCostUsd}</span>` : ''}
          </div>` : ''}
        </div>`;
      }).join('')}
    </div>`;
...
function showAgentDetail(id) {
  ...
  UI.modal(`${getAllRoleEmojis(a.role)} ${a.name}`, `
    ...
    ${a.totalTokens ? `<p><strong>Total Tokens:</strong> ${a.totalTokens}</p>` : ''}
    ${a.estimatedCostUsd ? `<p><strong>Est. Cost:</strong> $${a.estimatedCostUsd}</p>` : ''}
