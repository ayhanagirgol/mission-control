...
if (!DISPLAY_NAME) {
  console.error('Usage: create_mail_folder.js <displayName>');
  process.exit(2);
}
...
(async () => {
  ...
  const url = `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(MAILBOX_UPN)}/mailFolders('Inbox')/childFolders`;
  const folder = await graph(token, url, {
    method: 'POST',
