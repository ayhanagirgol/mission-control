...
if (!MESSAGE_ID) {
  console.error('Usage: get_message_info.js <messageId>');
  process.exit(2);
}
...
(async () => {
  ...
  const url = `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(MAILBOX_UPN)}/messages/${encodeURIComponent(MESSAGE_ID)}?$select=id,conversationId,subject,receivedDateTime`;
